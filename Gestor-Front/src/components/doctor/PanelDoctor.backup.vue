                ...
                <td>
                  <div class="btn-group" role="group">
                    ...
                    <button class="btn btn-sm btn-warning" 
                            ...
                            title="Marcar como No Acudió (1h después de la cita)">
                      ⊗ No Acudió
                    </button>
                    <button class="btn btn-sm btn-info"
                            ...
                            :title="puedeCrearReceta(c) ? 'Generar receta para este paciente' : 'Solo se pueden crear recetas el día de la cita'">
                      💊 Receta
                    </button>
                  ...
                  <span v-if="!puedeMarcarAtendida(c) && !puedeMarcarNoAsistio(c)" class="text-muted small">—</span>
                </td>
              </tr>
            </tbody>
          </table>
...
export default {
  ...
  methods:{
    ...
    puedeMarcarNoAsistio(c){
      // Solo mostrar botón si la cita está pagada y pendiente de atender
      const e = (c.Estatus||'').toLowerCase();
      ...
      try {
        ...
        const diferenciaMs = ahora - fechaHoraFin;
        const diferenciaHoras = diferenciaMs / (1000 * 60 * 60);
        
        return diferenciaHoras >= 1;
      } catch (error) {
        console.error('Error al validar tiempo para No Acudió:', error);
        return false;
      }
    },
    puedeCrearReceta(c){
      // Solo se puede crear receta si ya es la fecha de la cita
      ...
      try {
        ...
        // Solo permitir si hoy es igual o posterior a la fecha de la cita
        return hoyFecha >= citaFecha;
      } catch (error) {
        console.error('Error al validar fecha para receta:', error);
        return false;
      }
    },
    ...
    async marcarNoAsistio(c){
      ...
      const confirmar = confirm(
        `¿Confirmar que el paciente ${c.Paciente} NO ACUDIÓ a la cita del ${c.Fecha_cita_fmt || c.Fecha_cita}?\n\n` +
